...
// 千分号
function format(v) {
    // 1. 通过toLocaleString编译,可能根据本地语言有所差异
...
// 百分号
function wrapperPerrcentage(o, k) {
    return o && o[k] ? `${o[k]}%` : '0%'
}

// 利用计算属性机制,不断回调循环直到找到最后一个对象
function warpperObject(o, k) {
    if (o && k.indexOf('.') >= 0) {
        const keys = k.split('.')
        keys.forEach(key => {
            o = o[key]
...
export default {
    computed: {
        ...
        salesGrowthLastDay() {
            return wrapperPerrcentage(this.reportData, 'salesGrowthLastDay')
        },
        salesGrowthLastMonth() {
            return wrapperPerrcentage(this.reportData, 'salesGrowthLastMonth')
        },
        salesLastDay() {
        ...
        returnRate() {
            return wrapperPerrcentage(this.reportData, 'returnRate')
        },
        orderUserTrend() {
        ...
        category1() {
            return warpperObject(this.reportData, 'category.data1')
        },
        category2() {
            return warpperObject(this.reportData, 'category.data2')
        },
        mapData() {
