...
<script>
  ...
  module.exports = {
    ...
    methods: {
      loadMap() {
        ...
        if (typeof (window.L) != "undefined" && typeof (window.L.Proj) != "undefined" && this.id != "" && typeof (document.getElementById(this.id)) != "undefined") {
          ...
          this.loadFlag=this.loadFlag+1;
          if(this.loadFlag>3){
            alert("地图加载异常，请联系GIS剑谱维护人员");
            return;
          }
          ...
          this.loadFlag=this.loadFlag+1;
          if(this.loadFlag>3){
            console.log("地图加载异常，1：可能存在依赖js未正常引入  2：map所在的div未设置id");
            alert("地图加载异常，1：可能存在依赖js未正常引入  2：map所在的div未设置id");
            return;
          }else{
            window.setTimeout(this.loadMap, 200);
          }
        }
      },
      ...
      addMarker: function (markerJSON) {
        ...
        if (typeof (markerJSON.option) != "undefined" && typeof (markerJSON.option.draggable) != "undefined") {
      ...
      addCircle(circleJson) {
        if (this.map.coordinateType == "WGS84" && this.map.coordinateUnit == "DEGREE") {
          ...
          var circle = L.circle(circleJson.xy, circleJson.option);
          circle.layerType = "circle";
          circle.radius = circleJson.option.radius;
          circle.centerPoint = circle._latlng;
          circle.addTo(this.map);
        }
        return circle;
      ...
      addMovingMarker(movingMarkerJson) {
        ...
        var movingObj = {};
        movingObj.icon = movingMarkerJson.icon;
        movingObj.loop = false;
        movingObj.pointCallBack = movingMarkerJson.pointCallBack;

        var moving_mark = L.Marker.movingMarker(movingMarkerJson.xys, times, movingObj).addTo(the.map);
        ...
        moving_mark.on('end', function () {
          ...
          //cbc-edit-start
          console.log("到最后点位置");
          if (typeof (movingMarkerJson.endCallBack) != "undefined") {
            movingMarkerJson.endCallBack();
          }
          the.map.closePopup();
    ...
    mounted() {
      var date1 = new Date();
      var date2 = new Date("2124/04/23 23:59:59");
      if (date1.getTime() > date2.getTime()) {
        alert("地图在2124/04/23 23:59:59使用已到期！请联系管理人员");
      } else {
        this.loadMap();
      }
    }
